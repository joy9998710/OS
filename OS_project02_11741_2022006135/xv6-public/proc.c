...
Dequeue(struct queue *Q){
	...
	struct proc *rm_p = Q->arr[Q->front];
	Q->arr[Q->front] = NULL;
...
//    }while(idx != (Q->rear + 1) % Q->capacity);
//    return -1;
//}
// 
// int
// removeAt(struct queue *Q, int pid){
...
Decrease_queue(struct proc *p){
	...
	if(p->queue_level == 0){
		...
		p->queue_level = 3;
		p->ticks_used = 0;
		p->time_quantum = L3[p->priority].time_quantum;
		Enqueue(&L3[p->priority], p);
...
yield(void){
  ...
  if(is_moq && myproc() && myproc()->in_moq){
	  release(&ptable.lock);
	  return;
  }
  if(!is_moq && myproc() && myproc()->in_moq){
